...
router.get('/', verifyToken, async (req, res, next) => {
  try {
    const entities = await ingredientsService.getAll();
...
router.get('/:id', verifyToken, async (req, res, next) => {
  const { id } = req.params;
  try {
    const entity = await ingredientsService.getById(id);
...
router.post('/', verifyToken, async (req, res, next) => {
  try {
    const savedEntity = await ingredientsService.create(req.body);
...
router.put('/:id', verifyToken, async (req, res, next) => {
  try {
    const updatedEntity = await ingredientsService.update(id, req.body);
...
router.delete('/:id', verifyToken, async (req, res, next) => {
  try {
    const deletedEntity = await ingredientsService.deleteById(id);
